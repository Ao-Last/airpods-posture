...
It also plays matched audio feedback so movement has an immediate confirmation loop. Directional postures use simple equal-power stereo panning in the Lab: left turn / tilt cues lean into the left channel, right turn / tilt cues lean into the right channel, and cue strength follows posture offset, angular speed, and acceleration. The audio cue is part of the input surface, not decoration.
